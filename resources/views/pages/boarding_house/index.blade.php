...
@section('title', 'find kost results')
...
<div id="TopNav" class="relative flex items-center justify-between px-5 mt-[60px]">
    <a href="{{ route('find_kost')}}" class="w-12 h-12 flex items-center justify-center shrink-0 rounded-full overflow-hidden bg-white">
...
<div id="Header" class="relative flex items-center justify-between gap-2 px-5 mt-[18px]">
    <div class="flex flex-col gap-[6px]">
        ...
        <p class="text-ngekos-grey">Tersedia {{ number_format($boardingHouses->count())}} Kos</p>
    </div>
</div>
<section id="Result" class=" relative flex flex-col gap-4 px-5 mt-5 mb-9">
    @forelse ($boardingHouses as $item)
    <a href="{{ route('kost_show', $item->slug)}}" class="card">
        <div class="flex rounded-[30px] border border-[#F1F2F6] p-4 gap-4 bg-white hover:border-[#91BF77] transition-all duration-300">
            ...
            <div class="flex flex-col gap-3 w-full">
                ...
                <div class="flex items-center gap-[6px]">
                    ...
                    <p class="text-sm text-ngekos-grey">{{ $item->city->name }} City</p>
                </div>
                <div class="flex items-center gap-[6px]">
                    <img src="assets/images/icons/profile-2user.svg" class="w-5 h-5 flex shrink-0" alt="icon">
                    <p class="text-sm text-ngekos-grey">{{ $item->rooms->pluck('room_type')->min() }} - {{ $item->rooms->pluck('room_type')->max() }} People</p>
                </div>
                <hr class="border-[#F1F2F6]">
                <p class="font-semibold text-lg text-ngekos-orange">Rp {{ number_format($item->price, 0, ',', '.')}}<span class="text-sm text-ngekos-grey font-normal">/tahun</span></p>
            </div>
        </div>
    </a>
    @empty
    <p class="text-red-500 text-center">Maaf data tidak tersedia</p>
    @endforelse
