...
<div id="Gallery" class="swiper-gallery w-full overflow-x-hidden -mb-[38px]">
    <div class="swiper-wrapper">
        ...
        @foreach ($room->roomImages as $item)
...
<main id="Details" class="relative flex flex-col rounded-t-[40px] py-5 pb-[10px] gap-4 bg-white z-10">
    ...
    <div id="Features" class="grid grid-cols-2 gap-x-[10px] gap-y-4 px-5">
        ...
        <div class="flex items-center gap-[6px]">
            ...
            <p class="text-ngekos-grey">{{ $boardingHouse->rooms->pluck('room_type')->min() }} - {{ $boardingHouse->rooms->pluck('room_type')->max() }} People</p>
        </div>
        <div class="flex items-center gap-[6px]">
    ...
    <div id="TabsContent" class="px-5">
        <div id="Bonus-Tab" class="tab-content flex flex-col gap-5">
            <div class="flex flex-col gap-4">
                @forelse ($boardingHouse->bonuses as $item)
                <div
                    ...
                    <div>
                        ...
                        <p class="text-sm text-ngekos-grey">{!! $item->description !!}</p>
                    </div>
                </div>
                @empty
        ...
        <div id="Testimonials-Tab" class="tab-content flex-col gap-5 hidden">
            <div class="flex flex-col gap-4">
                @forelse ($boardingHouse->testimonials as $item)
                <div
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="font-semibold">{{ $item->name }}</p>
                            <p class="mt-[2px] text-sm text-ngekos-grey">{{ \Carbon\Carbon::parse($item->created_at)->format('d F Y')}}</p>
                        </div>
                    </div>
                    <p class="leading-[26px]">{!! $item->content !!}</p>
                    <div class="flex">
        ...
        <div id="Contact-Tab" class="tab-content flex-col gap-5 hidden">{!! $boardingHouse->address !!}</div>
...
<div id="BottomNav" class="relative flex w-full h-[138px] shrink-0">
    <div class="fixed bottom-5 w-full max-w-[640px] px-5 z-10">
        <div class="flex items-center justify-between rounded-[40px] py-4 px-6 bg-ngekos-black">
            ...
            <a href="{{ route('kost_rooms', $boardingHouse->slug)}}"
